...
/**
 * The Dungeon class implement the Model.DungeonInterface, in order to provide all the publicly
 * usable functionalities.
 * It represents the game basically. It has everything in it.
 * player, caves, tunnels etc.
 */
public class Dungeon implements DungeonInterface, Serializable {
   ...
   * @param player            The Model.Player.
   * @param warpAllowed       Is this dungeon Warping or not.
   */
  public Dungeon(String sizeRxC, int treasurePerc, int interConnectivity, Player player,
    ...
    if (treasurePerc < 0 || treasurePerc > 100) {
      throw new IllegalArgumentException("Invalid Model.Treasure Percentage.");
    }
  ...
  private void addTreasures(int percentage) {
    ...
    int totalTunnels = this.getAllTunnels().size();
    int totalNodes = totalTunnels + totalCaves;
    ...
    while (set.size() != treasureCaves) {
      //count += 1;
      int randCaveNum = ThreadLocalRandom.current().nextInt(randomCStart, totalNodes);
      ...
      //// will stop infinite loop, for if random numbers are repeating or any such error
      //if (count > 10000 ) {
      //  randomCStart += 1;
      //}
  ...
  /**
   * Return the list of treasure containers.
   *
   * @return The list of treasure containers.
   */
  public List<CaveTunnelInterface> getTreasureFullContainersList() {
    List<CaveTunnelInterface> list = new ArrayList<>();
    for (int i = 0; i < this.sizeOfDungeon[0]; i++) {
      for (int j = 0; j < this.sizeOfDungeon[1]; j++) {
  ...
  public void movePlayer(int[] newLocation) {
    if (this.player.isDead()) {
      throw new IllegalArgumentException("The player is dead, RIP Model.Player.");
    }
   ...
   *
   * @return List of all the Model.Graph.Edges  .
   */
  ...
  public int[] getDungeonSize() {
    return new int[]{this.grid2D.length, this.grid2D[0].length};
  }
