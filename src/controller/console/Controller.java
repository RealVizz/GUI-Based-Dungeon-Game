...
/**
 * The controller class for the Model.Dungeon Game.
 */
  ...
  public void playGame() {
    ...
    while (!dungeon.getPlayer().isDead() && (currLoc[0] != endLoc[0] || currLoc[1] != endLoc[1])) {
      try {
        ...
        this.out.append("\nDetails of Model.Player    : ").append(dungeon.getPlayer().toString());

        StringBuilder smellSB = new StringBuilder();
        if (dungeon.playerSmell() == 0) {
          smellSB.append("\nModel.Player do not smell anything pungent.");
        } else if (dungeon.playerSmell() == 1) {
          smellSB.append("\nModel.Player smells something mildly pungent nearby.");
        } else if (dungeon.playerSmell() == 2) {
          smellSB.append("\nModel.Player smells something heavily pungent nearby.");
        }

        this.out.append(smellSB);
        this.out.append("\nModel.Player Options : Move[M]    Pick[P]     Shoot[S]     Quit[Q]\n");
        ...
        switch (usrInput) {
          case "M":
            ...
            while (true) {
              ...
              if (move.equals("U") || move.equals("D") || move.equals("L") || move.equals("R")) {
                ...
                this.out.append("\nOk Jon, We No Elon or Jeff....we cant move in space..."
                        + "Try Again");
              }
            }

            if (dungeon.getPlayer().isDead()) {
              this.out.append("\nThe Model.Player is Eaten by the monster....Gone!!!");
            }
          ...
          case "P":
            ...
            pickSB.append("Model.Player Picked : ");
